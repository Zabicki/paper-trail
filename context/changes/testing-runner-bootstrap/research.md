...
6. **Does `getViteConfig` actually resolve `astro:env/server` under the Cloudflare
   adapter?** Astro's own docs do not state it, and the linked upstream issues concern
   integrations rather than Vitest. Phase 1 must verify empirically — but per §7 this
   does **not** block the first test.
