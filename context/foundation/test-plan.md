...
- **Reference tests**, two shapes. Point generation prompts at them **by path**
  rather than pasting them: a path cannot drift from the file it names, and a
  pasted sample makes an agent reproduce that specific flow instead of
  generalising from it.
  - `tests/e2e/seed.spec.ts` — the seed, and the model for a **single-surface**
    flow: write, read it back, reload, clean up through the UI. Start here.
  - `tests/e2e/expense-reaches-reports.spec.ts` — the model for a
    **cross-surface** flow, where one write is read back through a second path
    that shares nothing with the first. It saves one expense in the day view and
    asserts the Kategorie board attributes exactly that amount to exactly that
    category, i.e. across the `entries_category_summary` aggregate, a
    browser-resolved range, and `resolveDistribution`'s top-N model. That is the
    shape to copy whenever the risk is "the figure shown here disagrees with the
    figure shown there" — it is also the only current spec covering risk #2.
...
- **A spec that asserts an aggregate needs data nobody else can touch.** Reach
  for the `reportCategory` fixture, not `FIXTURE_CATEGORY`. The shared category
  accumulates whatever any spec leaves behind, so an expected total computed
  against it depends on the state of the account; `reportCategory` mints an
  expense category per run and soft-deletes it afterwards, which makes "the
  report's total for this category" equal "the amount this test entered". That
  is §6.1's oracle rule reaching this layer: the expectation stays hand-written
  arithmetic instead of a number read out of the app and compared against
  itself. Two consequences worth knowing before you copy it — the teardown is a
  soft delete, and `entries_category_summary` applies no `deleted_at` filter to
  the joined category, so what actually keeps the account clean is
  `entryDescription`'s row deletion, not this one; and the category-chip list
  collapses to five (`CategoryPicker.tsx`) and orders by recency, so a
  just-created category sits in the hidden tail — reach it through the
  `Szukaj kategorii` box, which the component documents as the escape hatch to
  any chip, hidden or not.
...
- **Prove the assertion, don't assume it.** Before a spec counts as done,
  ...
  **Break the figure as well as the row.** `expense-reaches-reports.spec.ts`
  needed two breaks, and the second is the one worth copying: dropping a row
  from the aggregate (`categories.splice(0, 1)`) proved the row-presence
  assertion, but only shifting every total by one grosz proved the _amount_
  assertion was carrying its own weight rather than riding on the row merely
  existing. An assertion that survives a wrong number is not protecting risk #2.
