...
| S-11 | `mobile-layout-fixes`                     | use every page on a phone without a sideways-shifted layout or a filter bar eating the viewport    | S-04, S-05       | FR-015, NFR (input friction)   | done   |
| S-12 | `topbar-tabs-and-receipt-date-button`     | see which section they are in from the top bar, and confirm a receipt on a phone without a gutter  | S-11, S-10       | NFR (input friction), FR-012   | done   |
...
| D      | Daily-use polish     | `S-07` → `S-09` → `S-10` · `S-08`, `S-11` → `S-12` parallel | Post-MVP pass over shipped surfaces, driven by real use rather than by the PRD. Joins Stream A at `S-03`, Stream B at `S-05`, Stream C at `S-06`. `S-11` is the first slice driven by testing on an actual phone rather than a narrow browser window; `S-12` is the second, and found the same symptom from a different mechanism. |
...
- **PRD refs:** FR-001, FR-002, Access Ctrl
  - FR-001 is marked covered here _only_ because the Baseline already satisfies it — this foundation adds the isolation half, not the sign-in half. FR-002 and the §Success Criteria strict-isolation guardrail are the real work.
- **Unlocks:** S-01, S-02, S-03, S-04, S-05, S-06 (every slice writes user-scoped rows). Reduces the blocking risk that FR-002 fails _silently_ — the failure mode `CLAUDE.md` and `tech-stack.md` both single out.
- **Prerequisites:** —
- **Parallel with:** —
- **Blockers:** —
- **Unknowns:** —
- **Risk:** One setup step is outstanding but not gating: the hosted Supabase project is live, but `supabase link` has not been run — it needs the DB password. Deliberately scoped to the _pattern_, not the whole schema: migration tooling, the RLS policy shape, and one table proving it. Prebuilding the full data model here would be horizontal work masquerading as a foundation, and would front-load decisions the later slices should make. The reason it is a foundation rather than folded into S-01 is safety: an RLS mistake leaks one user's finances to another and nothing errors, so the pattern must be verified before any real data exists.
- **Status:** done
...
### S-11: The app fits a phone

- **Outcome:** User can use every page on a phone with the layout squarely in the viewport — no sideways shift with a black gutter, the sign-out control inside the top bar rather than escaping it — and can scroll the Raporty charts without the filter bar occupying most of the screen.
- **Change ID:** `mobile-layout-fixes`
- **PRD refs:** FR-015, NFR (input friction — an off-viewport control is friction the ≤4-interaction budget cannot absorb)
- **Prerequisites:** S-04, S-05
- **Parallel with:** —
- **Blockers:** —
- **Unknowns:** —
- **Risk:** Two of the three reported symptoms were one defect. The top bar laid out `justify-between` with the user's email as a plain flex child; an email address has no spaces, so its min-content width is the whole string and the span cannot shrink. At 360px the bar's min-content came to 422px, the _document_ picked up a horizontal scrollbar, and because `bg-cosmic` paints one viewport-width while `body`'s near-black propagates to the canvas, the page read as shifted left of a black gutter. Measured before and after in headless Chromium at 320/360/390/768/1280 (`scrollWidth` vs `clientWidth`): 422/360 → 360/360. The fix is `flex-wrap-reverse` (so the links stack _above_ the email from a single DOM order, keeping identity before navigation for screen readers) plus `break-all` on the address. The third symptom is a deliberate reversal: the Raporty filter bar was pinned to the viewport top so that a recurring filter silently in effect off-screen could not make every figure below it quietly wrong — the FR-015 prominence requirement. Three stacked controls are ~180px, most of a phone viewport, so the bar now scrolls away and the invariant moved to the range caption, which names the active filter beside the range it applies to. **Anything that pins a control bar again inherits that trade.**
- **Status:** done — implemented directly on `master`, no change folder. Fixed files: `src/components/Topbar.astro`, `src/components/reports/ReportsView.tsx`.

### S-12: The top bar reads as tabs, and the receipt panel fits a phone

- **Outcome:** User can tell at a glance which section they are in — Dashboard and Raporty render as a segmented tab control with the current one highlighted, and `Wyloguj się` sits outside that group as the non-destination it is — and can confirm a parsed receipt on a phone without the page shifting sideways behind a black gutter.
- **Change ID:** `topbar-tabs-and-receipt-date-button`
- **PRD refs:** NFR (input friction — an unrecognisable control and an off-viewport layout both cost taps), FR-012
- **Prerequisites:** S-10, S-11
- **Parallel with:** —
- **Blockers:** —
- **Unknowns:** —
- **Risk:** The navigation half is presentation only: no route, no page and no auth path changed, and the active state is derived from `Astro.url.pathname` inside `Topbar.astro` rather than passed in per page, so a future page cannot forget to light its own tab. The overflow half is the S-11 symptom returning through a different mechanism — not an unshrinkable user string this time but `self-start` on the revert-date button, which drops the flex default of stretch and re-sizes the control to **max-content**; `Wróć do dnia z kalendarza (YYYY-MM-DD)` beside the 96px thumbnail then exceeded 360px. Fixed with `max-w-full` plus a shorter label (`Ustaw obecny dzień`) — the date left the label because the date field is directly above it. **The general rule now lives in `context/foundation/lessons.md`: anything using `self-start`, `w-fit` or `inline-flex` to avoid full width must carry `max-w-full`.**
- **Status:** done
...
| S-11       | `mobile-layout-fixes`                     | Top bar wraps on a phone; Raporty filter bar stops covering the viewport | —                                                     | n/a                   | Done directly on `master`, no plan or change folder |
| S-12       | `topbar-tabs-and-receipt-date-button`     | Top bar as a tab control; receipt revert-date button stops overflowing   | —                                                     | n/a                   | Done directly on `master`, no plan or change folder |

## Open Roadmap Questions

1. **Is bulk import of the existing Google Sheet really post-MVP?** — Owner: user. Block: `roadmap-wide` (does not gate planning, but changes S-04/S-05's value). The primary success criterion is retiring that sheet for 30 consecutive days, yet the history stays behind and the PRD's own non-goals concede that quick-select ranges "have little to range over in the first weeks." Carried from PRD OQ5 and _raised in priority here_, because sequencing has now made the consequence concrete: two of six slices ship into an empty dataset. — tracked in [#8](https://github.com/Zabicki/paper-trail/issues/8)
...
- **S-11: User can use every page on a phone with the layout squarely in the viewport, and can scroll the Raporty charts without the filter bar occupying most of the screen.** — Done 2026-08-18, directly on `master` with no plan, change folder or archive entry: three reported symptoms, two files, and no schema or API surface touched. Two of the three symptoms shared one root cause (an unshrinkable email span overflowing the document — see the S-11 slice for the measurements). The third reverses a deliberate S-04 decision: the reports filter bar is no longer pinned to the viewport. Lesson: a no-space string in a flex row overflows the whole document — see `context/foundation/lessons.md`.
- **S-12: User can tell which section they are in from the top bar, and can confirm a parsed receipt on a phone without the page shifting sideways.** — Done 2026-08-18, directly on `master` with no plan, change folder or archive entry: two files, presentation only, no route/schema/API surface touched. Fixed files: `src/components/Topbar.astro` (segmented tab group, active state from `Astro.url.pathname`, sign-out moved outside the group past a divider), `src/components/receipts/ReceiptReview.tsx` (`max-w-full` on the revert-date button, label shortened to `Ustaw obecny dzień`). Lesson: `self-start` re-sizes a control to max-content and re-opens the S-11 overflow through a different door — see `context/foundation/lessons.md`.
