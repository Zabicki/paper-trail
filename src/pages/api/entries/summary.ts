...
import { getEntriesSummary, RangeTooLargeError, summaryQuerySchema } from "@/lib/services/reports";

// Bucketed expense/income sums over a date range, plus the immediately
// preceding equal-length range in the same response — one round trip for
// everything /reports renders.
//
...
export const GET: APIRoute = async (context) => {
  ...
  // Validated after the auth guard, matching entries/categories.ts: an
  // anonymous caller should not be able to tell a malformed query string from
  // a missing session.
  ...
  // Lexicographic comparison is exact for zero-padded ISO dates, the same
  // trick days.ts:55-58 uses to clamp its range.
  ...
  try {
    const summary = await getEntriesSummary(supabase, parsed.data);
